...
function Get-TargetResource
{
    ...
    try
    {
        $currentAuditFlag = Get-AuditSubCategory -Name $GUID
        Write-Verbose -Message ( $localizedData.GetAuditpolSubcategorySucceed -f $GUID, $AuditFlag )
    }
    catch
    {
        Write-Verbose -Message ( $localizedData.GetAuditPolSubcategoryFailed -f $GUID, $AuditFlag )
    }
...
function Set-TargetResource
{
    ...
    if ( -Not ( Test-ValidSubcategory -Name $GUID ) )
    {
        Throw ( $localizedData.InvalidSubcategory -f $GUID )
    }

    try
    {
        Set-AuditSubcategory -Name $GUID -AuditFlag $AuditFlag -Ensure $Ensure
        Write-Verbose -Message ( $localizedData.SetAuditpolSubcategorySucceed `
                        -f $GUID, $AuditFlag, $Ensure )
    }
    catch
    {
...
function Test-TargetResource
{
    ...
    try
    {
        [String] $currentAuditFlag = Get-AuditSubCategory -Name $GUID
        Write-Verbose -Message ( $localizedData.GetAuditpolSubcategorySucceed -f $GUID, $AuditFlag )
    }
    catch
    {
...
function Get-AuditSubCategory
{
    ...
    $subcategory = Invoke-AuditPol -Command 'Get' -SubCommand "Subcategory:""$GUID"""
...
function Set-AuditSubcategory
{
    ...
    #>
    if ( $pscmdlet.ShouldProcess( "$GUID","Set AuditFlag '$AuditFlag'" ) )
    {
        ...
        {
            [String[]] $subcommand = @( "Subcategory:""$GUID""", "/success:$($auditState[$Ensure])" )
        }
        else
        {
            [String[]] $subcommand = @( "Subcategory:""$GUID""", "/failure:$($auditState[$Ensure])" )
        }
